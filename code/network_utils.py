...
def build_mlp(
    ...
    # This was helpful: https://discuss.pytorch.org/t/when-should-i-use-nn-modulelist-and-when-should-i-use-nn-sequential/5463/8
    
    layers = []
    layers.append(nn.Linear(input_size, size)) # add input layer
    
    # Add linear hidden layers with ReLU activation
    for i in range(n_layers):
        
        layers.append(nn.Linear(size, size))
        layers.append(nn.ReLU())
    
    layers.append(nn.Linear(size, output_size)) # add output layer
    
    
    model_list = nn.ModuleList(layers)
    my_model = nn.Sequential(*model_list) # pass tuple object to nn.Sequential
