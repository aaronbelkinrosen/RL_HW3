...
class BaselineNetwork(nn.Module): # class BaselineNetwork inherits from nn.Module
    """
    ...
    def __init__(self, env, config):
        ...
        input_layer = self.batch_size
        
        self.network = build_mlp(input_layer, self.batch_size, self.n_layers, self.layer_size)
    ...
    def forward(self, observations):
        ...
        output = self.forward(torch.flatten(observations))
        print('Executing forward in baseline network: ', observations.size)
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def calculate_advantage(self, returns, observations):
        ...
        Returns:
            advantages: np.array of shape [batch size] A(s,a) = E(r(s,a) - r(s))
        ...
        observed_returns = self.forward(observations.squeeze()) # Pass observations through network
        advantages = torch.sub(observed_returns, np2torch(returns))
    ...
    def update_baseline(self, returns, observations):
        ...
        Take error rate of forward propagate and feed loss backward through neural net layers
        ...
        observed_returns = self.forward(observations.squeeze()) # forward feed to get current error
        loss = torch.nn.functional.mse_loss(observed_returns, returns)
        loss.backward() # back propagate error to update parameters
        self.optimizer.step() # pass new parameters to optimizer
