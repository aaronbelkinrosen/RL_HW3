...
class BasePolicy:
    def action_distribution(self, observations):
        ...
        """
        dist_obj = ptd.distribution.Distribution(batch_shape=self.batch_size, event_shape=self.batch_size, validate_args=None)
        distribution = dist_obj.cdf(observations.squeeze())
        return distribution
...
class CategoricalPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        unnorm_outputs = self.network.forward(observations.squeeze()) # logits are just unnormalized outputs of a network after forward pass
        distribution = ptd.categorical.Categorical(probs=None, logits=unnorm_outputs, validate_args=None)
